CREATE OR REPLACE FUNCTION fn_update_cliente(
    ...
    wind_estado        tab_clientes.ind_estado%TYPE
) RETURNS BOOLEAN AS $$
BEGIN
    -- =============================================
    ...
    IF wval_diascartera < 0 OR wval_diascartera > 120 THEN  -- Cambié >=120 por >120
    ...
    IF NOT EXISTS (SELECT 1 FROM tab_terceros WHERE id_tercero = wid_cliente) THEN
        RAISE EXCEPTION 'El id_cliente % no existe como tercero. Debe registrarse primero en tab_terceros.', wid_cliente;
    END IF;
    ...
    -- =============================================
    -- VALIDACIÓN DE EXISTENCIA DEL CLIENTE (CORREGIDO)
    -- =============================================
    IF NOT EXISTS (SELECT 1 FROM tab_clientes WHERE id_cliente = wid_cliente) THEN
        RAISE EXCEPTION 'El cliente con ID % no existe', wid_cliente;
    END IF;
    ...
    UPDATE tab_clientes SET
        id_tipo           = wid_tipo,           -- Antes era ind_tipodoc
        fec_nacimi        = wfec_nacimi,
        val_edad          = NULL,               -- El trigger lo calculará
