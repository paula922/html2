...
(
    ...
    val_facactual       DECIMAL(6,0)        NOT NULL CHECK(val_facactual >= val_facini AND val_facactual <= val_facini),    ||||-- Valor actual de la factura 
...
(
    id_cliente          VARCHAR(20)         NOT NULL CHECK(LENGTH(TRIM(id_cliente)) >=6),	                                                                 --identificador del cliente							
...
(
	id_vendedor         VARCHAR(20)         NOT NULL CHECK(LENGTH(TRIM(id_vendedor)) >=6),                                                           --identificador del vendedor
...
(
	...
	id_vendedor         VARCHAR(20)         NOT NULL CHECK(LENGTH(TRIM(id_vendedor)) >=6),                              --identificador del vendedor
	fec_cotizacion      DATE                NOT NULL DEFAULT CURRENT_DATE,                                              --fecha de la creacion de la cotizacion
...
(
    ...
    val_observa         VARCHAR             NOT NULL CHECK(LENGTH(val_observa) >= 3),                               --observaciones de la cotizacion
...
(
	...
	id_vendedor         VARCHAR(20)         NOT NULL CHECK(LENGTH(TRIM(id_vendedor)) >=6),                                  --identificador del vendedor
    id_ciudad           VARCHAR	            NOT NULL CHECK(LENGTH(id_ciudad) = 5),                                          --identificador de la ciudad
	fec_factura         DATE                NOT NULL DEFAULT CURRENT_DATE,                                                  --fecha de la creacion de la factura
	id_formapago        DECIMAL(1,0)        NOT NULL CHECK(id_formapago >= 0 AND id_formapago <= 9),                        --identificador de la forma de pago
...
(
    ...
    cufe                VARCHAR(100)        NOT NULL UNIQUE,                                                                                --Código Único de Factura Electrónica (CUFE) generado por el sistema de facturación electrónica    
    ...
    fec_envio_dian      DATE                NOT NULL CHECK(fec_cotizacion = CURRENT_DATE),                                                  --Fecha de envío de la factura electrónica a la DIAN
...
(
    ...
    fec_pago               DATE                          NOT NULL DEFAULT CURRENT_DATE,									            --fecha  a realizar el pago
...
(
	...
	fec_seg             DATE                NOT NULL DEFAULT CURRENT_DATE,																						        --fecha en ques e hace el seguimiento de cartera
...
(
    ...
    id_factura             DECIMAL(6,0)                   NOT NULL CHECK(id_factura >=0 AND id_factura<=999999),																--identificador de la factura
    id_cartera             DECIMAL(5,0)                   NOT NULL CHECK(id_cartera >=0 AND id_cartera <=99999),																--identificador de la cartera
    fec_pago               DATE                           NOT NULL DEFAULT CURRENT_DATE,     																			        --fecha de Realizacion del pago
    ...
    ind_est_pag            VARCHAR                        NOT NULL,CHECK(ind_est_pag IN('APROVADO','RECHAZADO','PENDIENTE')),													--estado de pago
...
(
	...
	id_factura          DECIMAL(6,0)    NOT NULL CHECK(id_factura>=0 AND id_factura<=999999),							--identificador de la factura
	fec_emision         DATE            NOT NULL DEFAULT CURRENT_DATE,									                --fecha que se hace la devolución
	val_motivo          VARCHAR         NOT NULL CHECK(LENGTH(val_motivo)<=255),										--motivo por que se hace devolucion
	val_devolucion      DECIMAL(12,0)   NOT NULL CHECK(val_devolucion>=0 AND val_devolucion<=999999999999),			    --valor de la devolución
...
CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE OR DELETE ON faccar.tab_not_dvols
FOR EACH ROW EXECUTE FUNCTION public.fun_audit_trail();
