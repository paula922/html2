...
(
    ...
    ind_met_pago           VARCHAR                        NOT NULL,CHECK( ind_met_pago IN('EFECTIVO','PSE','TARJETA CREDITOO','TARJETA DEBITO','TRANSFERENCIAS')),		        --metodo de realizacion del pago
...
--------------------------------------
-- TABLLA DE NOTAS DEBITO Y CREDITO --
--------------------------------------
CREATE TABLE IF NOT EXISTS faccar.tab_notas 
(
	id_notaCredito      VARCHAR         NOT NULL CHECK(LENGTH(id_notaCredito) = 12),								    --identificador de la nota credito	
	id_factura          DECIMAL(6,0)    NOT NULL CHECK(id_factura>=0 AND id_factura<=999999),							--identificador de la factura
	fec_emision         DATE            NOT NULL DEFAULT CURRENT_DATE,	                                                --fecha de emision de la nota credito
    ind_tipo            VARCHAR         NOT NULL CHECK(ind_tipo IN('DEBITO','CREDITO')),								--tipo de nota  
