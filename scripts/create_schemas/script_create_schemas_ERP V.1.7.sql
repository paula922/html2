...
DROP TABLE IF EXISTS tab_workflow_expediente;
DROP SEQUENCE IF EXISTS seq_workflow_expediente;
DROP TABLE IF EXISTS tab_permiso_acceso;
...
DROP TABLE IF EXISTS tab_accion_workflow_doc;
...
DROP TABLE IF EXISTS tab_caja_menor;
...
DROP TABLE IF EXISTS tab_interacciones_lead;
DROP TABLE IF EXISTS tab_lead_camp;
DROP TABLE IF EXISTS tab_segmentacion_tercero;
...
DROP TABLE IF EXISTS tab_condiciones_regla;
...
DROP TABLE IF EXISTS tab_partici_programa;
DROP TABLE IF EXISTS tab_programa_salud;
DROP TABLE IF EXISTS tab_epp_inspeccion;
DROP TABLE IF EXISTS tab_epp_asignacion;
DROP TABLE IF EXISTS tab_categoria_epp; 
...
(
    ...
    nom_tipo        VARCHAR             NOT NULL,
    PRIMARY KEY(id_tipo)
);

CREATE TABLE tab_tel_prefijo
(
    id_prefijo      VARCHAR     NOT NULL,
...
(			
	id_tercero			VARCHAR(10)     	NOT NULL,		                                                            --identificador de terceros
...
(
    ...
    ind_tip_producto    BOOLEAN         NOT NULL DEFAULT TRUE,                                                             -- TRUE = comercial / FALSE = interno
    ind_tipo_bien       BOOLEAN         NOT NULL DEFAULT TRUE,                                                             -- TRUE = Bien  / FALSE = Servicio
...
(
    ...
    ano                     INTEGER               NOT NULL UNIQUE DEFAULT '2026',         -- Año de vigencia
    ...
    porc_calculo_ibc        DECIMAL(3,0)          NOT NULL DEFAULT 40.00,                 -- Porcentaje calculo IBC (40%)
...
(
    ...
    nom_escolar         VARCHAR(20)   NOT NULL DEFAULT 'SIN ESCOLARIDAD' CHECK(nom_escolar IN ('BACHILLER','AUXILIAR','TECNICO','TECNOLOGO','PROFESIONAL','ESPECIALISTA','MAGISTER','DOCTORADO')),-- Nombre del nivel educativo (ej:     
    ind_borrado         BOOLEAN       NOT NULL DEFAULT FALSE,    
...
(
    ...
    nom_cargo               VARCHAR(20)            NOT NULL DEFAULT 'SIN CARGO',                    -- Nombre del cargo
    ind_borrado             BOOLEAN                NOT NULL DEFAULT FALSE,
...
(
    ...
    tipo_documento          VARCHAR(20)             NOT NULL DEFAULT 'SIN TIPO DE DOCUMENTO DE CANDIDATO' CHECK(tipo_documento IN ('C.C','T.I','C.E','P.T','P.E','P.A')),   -- Tipo de documento del candidato (C.C, C.E, PTT, etc)  PENDIEMTE MIRAR TABLA NUEVA DE TIPO DE DOCUMENTO                  
...
-- DATOS PERSONALES
    nom_empleado            VARCHAR                NOT NULL DEFAULT 'SIN NOMBRE DE EMPLEADO',      -- Nombres del empleado
    ape_empleado            VARCHAR                NOT NULL DEFAULT 'SIN APELLIDO DE EMPLEADO',           -- Apellidos del empleado
    tipo_documento          VARCHAR                NOT NULL DEFAULT 'SIN TIPO DE DOCUMENTO DE EMPLEADO',                 -- Tipo de documento del empleado (C.C, C.E, PTT, etc)
    tipo_sangre             VARCHAR                NOT NULL DEFAULT 'SIN TIPO DE SANGRE DE EMPLEADO',           -- Tipo de samgre del empleado
    ...
    estado_civil            VARCHAR                NOT NULL DEFAULT 'SIN ESTADO CIVIL DE EMPLEADO',             -- Estado civil del empleado (Soltero, casado, etc)
--DATOS GEOGRÁFICOS
    dir_empleado            DATOS_UBICACION        NOT NULL,                               -- Datos de contacto del empleado
-- DATOS FAMILIARES
    nom_conyuge             VARCHAR                NOT NULL DEFAULT 'SIN NOMBRE CÓNYUGE EMPLEADO',           -- Nombre conyuge del empleado
    num_hijos               DECIMAL(2,0)           NOT NULL DEFAULT 00,                    -- Número de hijos del empleado
    nom_contac_emer         VARCHAR                NOT NULL DEFAULT 'SIN NOMBRE CONTACTO EMERGENCIA EMPLEADO',           -- Nombre del contacto de emergencia del empleado
...
-- DATOS LABORALES
    ...
    nom_eps                 VARCHAR(10)            NOT NULL DEFAULT 'SIN NOMBRE EPS DE EMPLEADO',           -- nombre de la eps del empleado
    nom_ccf                 VARCHAR(10)            NOT NULL DEFAULT 'SIN NOMBRE CCF DE EMPLEADO',           -- nombre caja de compensasión del empleado
    nom_afp                 VARCHAR(10)            NOT NULL DEFAULT 'SIN NOMBRE AFP DE EMPLEADO',           -- nombre del fondo de pension del empleado  
    nom_arl                 VARCHAR(10)            NOT NULL DEFAULT 'SIN NOMBRE ARL DE EMPLEADO',           -- nombre ARL del empleado
    ...
    id_banco                DECIMAL(10,0)          NOT NULL,                               --nombre de banco para pago 
    num_cuenta              DECIMAL(15,0)          NOT NULL,                               -- numero de cuenta bancaria para pagos
...
(
    ...
    nom_concepto            VARCHAR(50)                     NOT NULL,                       --Salario, Hora Extra, Recargo Nocturno, Incapacidad, Prima, o Deducción.
    ...
    val_concepto            VARCHAR(15)                     NOT NULL DEFAULT 'no aplica',
    ind_borrado             BOOLEAN                         NOT NULL DEFAULT FALSE,
...
(
    ...
    ind_borrado             BOOLEAN                         NOT NULL DEFAULT FALSE,
    PRIMARY KEY(ano_nomina, mes_nomina, val_periodo, id_empleado, id_concepto)                                           -- Relación concepto
);
...
(
    ...
    val_neto            DECIMAL(10,0)   NOT NULL CHECK(val_neto >= 0),
    ind_borrado             BOOLEAN                         NOT NULL DEFAULT FALSE,
...
(
    ...
    obser_tipo_acc         	TEXT                NOT NULL        DEFAULT 'N/A',                                                          --OBSERVACIONES DEL ACCIDENTE
    ind_borrado				BOOLEAN				NOT NULL 		DEFAULT FALSE, 						  
...
(
	...
	id_categoria_epp        DECIMAL(1,0) 		NOT NULL,       CHECK(id_categoria_epp >= 1 AND id_categoria_epp <= 9),                 --IDENTIFICADOR DE LA CATEGORIA DE EPP 
...
(
    ...
    id_categoria_epp            DECIMAL(1,0)    NOT NULL   		CHECK   (id_categoria_epp >= 1 AND id_categoria_epp <= 9),      -- CATEGORÍA DEL EPP (REFERENCIA A LA TABLA DE CATEGORÍA DE EPP)
    ...
    desc_epp             	    TEXT            NOT NULL        DEFAULT 'N/A', 									                -- DESCRIPCIÓN DETALLADA DEL EPP: MATERIALES, ESPECIFICACIONES TÉCNICAS, USOS RECOMENDADOS
...
(
    id_programa           DECIMAL(2,0)                                NOT NULL      CHECK (id_programa >=1 AND id_programa <= 99),		    			    -- Indicador del programa de salud.
...
(
    id_programa          DECIMAL(2,0)                                NOT NULL       CHECK (id_programa >=1 AND id_programa <= 99),                          -- Identificador del programa de salud
    id_empleado          VARCHAR(10)                                 NOT NULL       CHECK (LENGTH(id_empleado) >= 6 AND LENGTH(id_empleado) <= 10 ),        -- Identificador del empleado
    fecha_partici        DATE                                        NOT NULL       CHECK (fecha_partici <= CURRENT_DATE),                                  -- Fecha de participación en el programa, llevar asistencia de empleados
    ind_asistencia       BOOLEAN                                     NOT NULL,                                                                              -- TRUE = Asistió, FALSE = No hubo asistencia.
	ind_borrado			 BOOLEAN									 NOT NULL 		DEFAULT FALSE,	
...
(
    ...
    nom_tema                VARCHAR                               NOT NULL         CHECK(LENGTH(nom_tema) >= 0 AND LENGTH(nom_tema) <= 60),        -- Nombre de la capacitación o programa de salud que realizarán.
    ind_borrado				BOOLEAN								  NOT NULL 		   DEFAULT FALSE,	
...
(
  ...
  correo_docente            VARCHAR                               NOT NULL        CHECK (correo_docente LIKE '%@%' AND correo_docente LIKE '%.%'),         -- Correo electrónico del docente.
...
INSERT INTO tab_etapas_funnel (id_etapa, nom_etapa, des_etapa, ind_etapa_final, ind_estado, ind_borrado)VALUES
    (1, 'Prospecto',FALSE, TRUE, FALSE),
    (2, 'Contactado',FALSE, TRUE, FALSE),
    (3, 'Interesado',FALSE, TRUE, FALSE),
    (4, 'Propuesta enviada',FALSE, TRUE, FALSE),
    (5, 'Negociación',FALSE, TRUE, FALSE),
    (6, 'Ganado',TRUE, TRUE, FALSE),
    (7, 'Perdido',TRUE, TRUE, FALSE);
...
INSERT INTO tab_canales (id_canal, nom_canal, des_canal, ind_estado, ind_borrado) VALUES
    (1, 'Email',TRUE, FALSE),
    (2, 'Instagram',TRUE, FALSE),
    (3, 'Facebook',TRUE, FALSE),
    (4, 'LinkedIn',TRUE, FALSE),
...
INSERT INTO tab_criterios_segmentacion (id_criterio, nom_criterio, des_criterio, ind_estado, ind_borrado) VALUES
    (1, 'Origen', TRUE, FALSE),             --Define de dónde proviene el lead o cliente'
    (2, 'Temperatura',TRUE,FALSE),          -- Indica qué tan cerca está el lead de tomar una decisión de compra
    (3, 'Tamaño de empresa',TRUE, FALSE),   -- Clasifica según el tamaño de la empresa del lead o cliente
    (4, 'Otro',TRUE, FALSE);                --Criterio adicional configurable según las necesidades del negocio
...
INSERT INTO tab_valores_segmentacion (id_criterio, id_valor, nom_valor, val_peso, ind_estado) VALUES
    -- Origen (id_criterio = 1)
    (1, 1, 'Redes Sociales',  2.00, TRUE),
    (1, 2, 'Referido',        3.00, TRUE),
    (1, 3, 'Búsqueda Orgánica',1.00, TRUE),
    (1, 4, 'Email',           2.00, TRUE),
    -- Temperatura (id_criterio = 2)
    (2, 1, 'Frío',   1.00, TRUE),
    (2, 2, 'Tibio',  2.00, TRUE),
    (2, 3, 'Caliente',3.00, TRUE),
    -- Tamaño de empresa (id_criterio = 3)
    (3, 1, 'Pequeña', 1.00, TRUE),
    (3, 2, 'Mediana', 2.00, TRUE),
    (3, 3, 'Grande',  3.00, TRUE);
...
INSERT INTO tab_campos_segmentacion_marcom (id_campo, id_tabla, id_atributo, ind_estado, ind_borrado) VALUES
    (1, 1, 1, TRUE, FALSE),  -- tab_leads_id_canal
    (2, 1, 2, TRUE, FALSE),  -- tab_leads_id_etapa
    (3, 2, 1, TRUE, FALSE);  -- tab_terceros.ind_tipo_tercero
...
INSERT INTO tab_reglas_segmentacion(id_regla, id_criterio, id_valor, tipo_logica, ind_estado)VALUES
...
(
    id_operador     DECIMAL(2,0)    GENERATED ALWAYS AS IDENTITY (START WITH 1 INCREMENT BY 1 MINVALUE 1 MAXVALUE 99),
    nom_operador    VARCHAR(3)      NOT NULL CHECK(LENGTH(TRIM(nom_operador)) >= 1),
    ind_estado      BOOLEAN         NOT NULL DEFAULT TRUE,
    ind_borrado     BOOLEAN         NOT NULL DEFAULT FALSE,
...
(
    ...
    id_campo        DECIMAL(3,0)    NOT NULL CHECK(id_campo > 0 AND id_campo <= 999),
    id_operador     DECIMAL(2,0)    NOT NULL CHECK(id_operador > 0 AND id_operador <= 99),
    val_condicion   TEXT            NOT NULL CHECK(LENGTH(TRIM(val_condicion)) > 0),
    PRIMARY KEY(id_regla, id_condicion),
    FOREIGN KEY(id_regla)    REFERENCES tab_reglas_segmentacion_marcom(id_regla),
...
(
    id_campana      DECIMAL(6,0)    GENERATED ALWAYS AS IDENTITY (START WITH 1 INCREMENT BY 1 MINVALUE 1 MAXVALUE 999999),
    id_tipo_campana DECIMAL(2,0)    NOT NULL CHECK(id_tipo_campana > 0 AND id_tipo_campana <= 99),
...
(
    id_motivo_perdida   DECIMAL(2,0)    GENERATED ALWAYS AS IDENTITY (START WITH 1 INCREMENT BY 1 MINVALUE 1 MAXVALUE 99), -- Corregido: >= 0 para permitir el registro especial
...
(
    ...
    PRIMARY KEY(id_lead),
    FOREIGN KEY(id_lead)           REFERENCES tab_terceros(id_tercero),
...
(
    id_interaccion DECIMAL(10,0) GENERATED ALWAYS AS IDENTITY (START WITH 1 INCREMENT BY 1 MINVALUE 1 MAXVALUE 9999999999),
    nom_tipo_interaccion    VARCHAR(50)     NOT NULL CHECK(LENGTH(TRIM(nom_tipo_interaccion)) >= 3),
    ind_estado              BOOLEAN         NOT NULL DEFAULT TRUE,
    ind_borrado             BOOLEAN         NOT NULL DEFAULT FALSE,
    
    PRIMARY KEY(id_tipo_interaccion)
);

INSERT INTO tab_tipos_interaccion_marcom (id_tipo_interaccion, nom_tipo_interaccion, ind_estado, ind_borrado) VALUES
    (1, 'Llamada',      TRUE, FALSE),
    (2, 'Mensaje',      TRUE, FALSE),
    (3, 'Reunión',      TRUE, FALSE),
    (4, 'Cotización',   TRUE, FALSE),
    (5, 'Email',        TRUE, FALSE);
...
(
    id_accion       DECIMAL(10,0)   GENERATED ALWAYS AS IDENTITY (START WITH 1 INCREMENT BY 1 MINVALUE 1 MAXVALUE 9999999999),
...
(
    id_campana                  DECIMAL(6,0)    NOT NULL CHECK(id_campana > 0 AND id_campana <= 999999),        --VALIDA LA MEDICION DE UN KPI POR PERIODO, EN ESTE CASO MENSUAL
    id_kpi                      DECIMAL(2,0)    NOT NULL CHECK(id_kpi > 0 AND id_kpi <= 99),
...
(
    id_plantilla        DECIMAL(4,0)    GENERATED ALWAYS AS IDENTITY (START WITH 1 INCREMENT BY 1 MINVALUE 1 MAXVALUE 9999),
    id_canal            DECIMAL(2,0)    NOT NULL CHECK(id_canal > 0 AND id_canal <= 99),
...
-- TIPOS DE NOTA
-- =============================================================================
CREATE TABLE IF NOT EXISTS tab_tip_notas
(
    id_tipo_nota        DECIMAL(1,0)    NOT NULL CHECK(id_tipo_nota IN (1, 2)),                 -- 1 = Nota Crédito (NC) | 2 = Nota Débito (ND)
    nom_tipo_nota       VARCHAR(30)     NOT NULL CHECK(LENGTH(TRIM(nom_tipo_nota)) >= 3),       -- Descripción del tipo de nota (Ej: "Nota de Crédito" o "Nota de Débito")
    afecta_inventario   BOOLEAN         NOT NULL DEFAULT FALSE,                                 -- TRUE: La nota afecta inventario  / FALSE: No afecta inventario
    afecta_tercero      BOOLEAN         NOT NULL DEFAULT TRUE,                                  -- TRUE: La nota afecta tercero / FALSE: No afecta tercero 
    ind_activo          BOOLEAN         NOT NULL DEFAULT TRUE,                                  -- Solo los tipos de nota activos pueden ser usados en la creación de notas /false para inactivar un tipo de nota sin perder la información histórica
    ind_borrado         BOOLEAN         NOT NULL DEFAULT FALSE,                                 -- TRUE: Borrado lógico (Inactivo) / FALSE: Activo
     PRIMARY KEY(id_tipo_nota)
);
 
CREATE INDEX IF NOT EXISTS idx_tip_notas_activo ON tab_tip_notas(ind_activo);
...
(
    ...
    id_tipo_nota        DECIMAL(1,0)    NOT NULL CHECK(id_tipo_nota IN (1, 2)),                 -- Relación con el tipo de nota (NC o ND)
    ...
    ind_activo          BOOLEAN         NOT NULL DEFAULT TRUE,                                  -- Solo los motivos activos pueden ser usados en la creación de notas
    ind_borrado         BOOLEAN         NOT NULL DEFAULT FALSE,                                 -- TRUE: Borrado lógico (Inactivo) / FALSE: Activo
    PRIMARY KEY(id_motivo_nota),
    FOREIGN KEY(id_tipo_nota)  REFERENCES tab_tip_notas(id_tipo_nota),
 
    UNIQUE(id_tipo_nota, cod_dian)
);
...
(
    ...
    id_tipo_nota        DECIMAL(1,0)    NOT NULL CHECK(id_tipo_nota IN (1, 2)),                                                                                         -- Relación con el tipo de nota (1=NC, 2=ND)
    id_motivo_nota      DECIMAL(3,0)    NOT NULL CHECK(id_motivo_nota > 0),                                                                                             -- Relación con el motivo de la nota (ver tabla tab_motivo_nota)
    id_empresa          DECIMAL(10,0)   NOT NULL CHECK(id_empresa >= 1000000000 AND id_empresa <= 9999999999),                                                          -- Relación con la empresa emisora de la nota 
    ...
    UNIQUE(id_empresa, id_tipo_nota, prefijo_nota, num_nota)
);
...
(
    ...
    id_factura              VARCHAR(10)     NOT NULL CHECK(id_factura IS NULL OR LENGTH(TRIM(id_factura)) >= 8),                -- Relación con la factura a la que se aplica la nota. Puede ser NULL si la nota se aplica a cartera en lugar de a una factura específica.
    id_cartera              DECIMAL(5,0)    NOT NULL CHECK(id_cartera IS NULL OR (id_cartera >= 0 AND id_cartera <= 99999)),    -- Relación con la cartera a la que se aplica la nota. Puede ser NULL si la nota se aplica a una factura específica en lugar de a cartera.
...
(
    ...
    nom_festivo        VARCHAR(30)          NOT NULL CHECK((LENGTH(nom_festivo) >= 3) AND (LENGTH(nom_festivo) <= 30)),          -- Nombre descriptivo
    ind_borrado        BOOLEAN              NOT NULL DEFAULT FALSE,                   -- TRUE: Borrado lógico (Inactivo) / FALSE: Activo
...
(
    ...
    FOREIGN KEY         (id_caja_menor)     REFERENCES tab_caja_menor(id_caja_menor)
);
...
(
    ...
    id_festivo           DECIMAL(2,0)               NOT NULL CHECK (id_festivo >= 1 AND id_festivo <= 99) DEFAULT 0,   -- FK al calendario de festivos. Define si se cuentan dias naturales o habiles.
...
--------------------------------------------------------------------------------------------------------
-- TABLA: tab_accion_workflow_doc
-- Define las acciones que se pueden ejecutar sobre un documento dentro de su flujo de trabajo.
-- Permite registrar que acciones se realizan sobre los documentos (crear, asignar, revisar,
-- aprobar, rechazar, publicar, archivar, eliminar) para la trazabilidad y auditoria.
--------------------------------------------------------------------------------------------------------
...
(
    ...
    nom_accion           VARCHAR(15)                NOT NULL CHECK (char_length(trim(nom_accion)) BETWEEN 3 AND 15),     -- Accion: CREAR, ASIGNAR, REVISAR, APROBAR, RECHAZAR, PUBLICAR, ARCHIVAR, ELIMINAR.
    ind_borrado          BOOLEAN                    NOT NULL DEFAULT FALSE,                                              -- Borrado logico.
    PRIMARY KEY (id_accion)
);
--------------------------------------------------------------------------------------------------------
...
-- Almacena los metadatos detallados de cada folio o pagina dentro de un radicado.
-- Permite etiquetar, clasificar y asociar metadatos (numero de folio, version,
-- palabras clave) a cada pagina de un documento. Facilita la busqueda y recuperacion
-- de informacion mediante palabras clave, y permite el control de versiones por folio.
--------------------------------------------------------------------------------------------------------
...
(
    ...
    num_folio            INTEGER                    NOT NULL CHECK (num_folio >= 1),                              -- Numero de folio dentro del radicado. Comienza en 1.
    num_version          INTEGER                    NOT NULL DEFAULT 1 CHECK (num_version >= 1),                  -- Numero de version del folio. Por defecto 1.
    palabras_clave       VARCHAR                    NOT NULL DEFAULT 'SIN_PALABRAS_CLAVES',                       -- Palabras clave para busqueda. Ej: "contrato,2024".
    id_retencion         DECIMAL(5,0)               NOT NULL CHECK (id_retencion BETWEEN 1 AND 9999),             -- FK a politica de retencion. Puede diferir del radicado padre.
    fec_indexacion       TIMESTAMP WITH TIME ZONE   NOT NULL DEFAULT now(),                                       -- Fecha y hora de indexacion. Por defecto now().
    observaciones        TEXT                       NOT NULL DEFAULT 'SIN_OBSERVACIONES',                         -- Observaciones adicionales.
    ind_borrado          BOOLEAN                    NOT NULL DEFAULT FALSE,                                       -- Borrado logico.
...
(
	id_expediente        VARCHAR(50)                NOT NULL CHECK (id_expediente IS NULL OR id_expediente ~ '^[A-Z0-9_]{5,50}$'),           -- FK al expediente (si aplica).
...
--------------------------------------------------------------------------------------------------------
-- SECUENCIA: seq_permiso_acceso
-- Proposito: Genera identificadores unicos para cada permiso de acceso.
--------------------------------------------------------------------------------------------------------
CREATE SEQUENCE IF NOT EXISTS seq_permiso_acceso START WITH 1 INCREMENT BY 1 NO CYCLE;
--------------------------------------------------------------------------------------------------------
-- TABLA: tab_permiso_acceso
-- Gestiona los permisos que los usuarios tienen sobre los expedientes y la correspondencia.
-- Define que usuario puede acceder a que recurso (expediente o correspondencia),
-- que tipo de permiso tiene (lectura, escritura, aprobacion, eliminacion, administracion)
-- y durante que periodo de tiempo.
--------------------------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS tab_permiso_acceso
(
    id_permiso           DECIMAL(12,0)              NOT NULL DEFAULT nextval('seq_permiso_acceso'),                                             -- Identificador unico. Clave primaria.
	id_expediente        VARCHAR(50)                NOT NULL CHECK (id_expediente IS NULL OR id_expediente ~ '^[A-Z0-9_]{5,50}$'),           -- FK al expediente (si aplica).
    id_correspondencia   VARCHAR(14)                         CHECK (id_correspondencia IS NULL OR id_correspondencia ~ '^[0-9]{14}$'),                   -- FK a correspondencia (si el permiso es sobre correspondencia).
    id_usuario           VARCHAR                    NOT NULL CHECK(LENGTH(id_usuario) >= 5),                                                    -- FK al usuario. Minimo 5 caracteres.
    val_permiso          VARCHAR(15)                NOT NULL CHECK (val_permiso IN ('LECTURA','ESCRITURA','APROBACION','ELIMINACION','ADMIN')), -- Tipo de permiso.
    fec_inicio           DATE                       NOT NULL DEFAULT CURRENT_DATE,                                                              -- Fecha de inicio de vigencia.
    fec_fin              DATE                       NOT NULL CHECK (fec_fin >= fec_inicio),                                                     -- Fecha de fin de vigencia.
    ind_borrado          BOOLEAN                    NOT NULL DEFAULT FALSE,                                                                     -- Borrado logico.
    PRIMARY KEY (id_permiso),
    CONSTRAINT chk_permiso_un_recurso CHECK ((CASE WHEN id_expediente IS NOT NULL THEN 1 ELSE 0 END) + (CASE WHEN id_correspondencia IS NOT NULL THEN 1 ELSE 0 END) = 1),
    FOREIGN KEY (id_expediente)      REFERENCES tab_expediente(id_expediente),
    FOREIGN KEY (id_correspondencia) REFERENCES tab_correspondencia(id_correspondencia),
    FOREIGN KEY (id_usuario)         REFERENCES tab_usuarios(id_usuario)
);
-- Indices para optimizar consultas por usuario, expediente y correspondencia.
...
--------------------------------------------------------------------------------------------------------
-- SECUENCIA: seq_workflow_expediente
-- Proposito: Genera identificadores unicos para cada registro de workflow de expedientes.
--------------------------------------------------------------------------------------------------------
CREATE SEQUENCE IF NOT EXISTS seq_workflow_expediente START WITH 1 INCREMENT BY 1 NO CYCLE;
--------------------------------------------------------------------------------------------------------
-- TABLA: tab_workflow_expediente
-- Registra el flujo de trabajo (transiciones de estado) de expedientes y correspondencia.
-- Permite la trazabilidad completa de los cambios de estado que sufren los expedientes
-- y las comunicaciones. Registra que accion se ejecuto, desde que estado, hacia que estado,
-- quien la hizo y cuando.
--------------------------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS tab_workflow_expediente
(
    id_workflow          DECIMAL(18,0)              NOT NULL DEFAULT nextval('seq_workflow_expediente'),                            -- Identificador unico. Clave primaria.
    id_expediente        VARCHAR(50)                         CHECK (id_expediente IS NULL OR id_expediente ~ '^[A-Z0-9_]{5,50}$'),           -- FK al expediente (si aplica).
    id_correspondencia   VARCHAR(50)                         CHECK (id_correspondencia IS NULL OR id_correspondencia ~ '^[A-Z0-9_]{5,50}$'), -- FK a correspondencia (si aplica).
    id_accion            DECIMAL(2,0)               NOT NULL CHECK (id_accion BETWEEN 1 AND 99),                                    -- FK a la accion ejecutada.
    ...
    id_usuario           VARCHAR                    NOT NULL CHECK(LENGTH(id_usuario) >= 5),                                        -- FK al usuario que ejecuto la accion.
    ind_borrado          BOOLEAN                    NOT NULL DEFAULT FALSE,                                                         -- Borrado logico.
    PRIMARY KEY (id_workflow),
    CONSTRAINT chk_workflow_un_recurso CHECK ((CASE WHEN id_expediente IS NOT NULL THEN 1 ELSE 0 END) + (CASE WHEN id_correspondencia IS NOT NULL THEN 1 ELSE 0 END) = 1),
    FOREIGN KEY (id_expediente)      REFERENCES tab_expediente(id_expediente),
    FOREIGN KEY (id_correspondencia) REFERENCES tab_correspondencia(id_correspondencia),
    FOREIGN KEY (id_accion)          REFERENCES tab_accion_workflow_doc(id_accion),
    FOREIGN KEY (id_estado_origen)   REFERENCES tab_estado_correspondencia(id_estado),
    FOREIGN KEY (id_estado_destino)  REFERENCES tab_estado_correspondencia(id_estado)
);
-- Indices para optimizar consultas por expediente, correspondencia y usuario.
...
(
    ...
    nom_pqr              VARCHAR(10)                NOT NULL CHECK (char_length(trim(nom_pqr)) BETWEEN 1 AND 10),
...
-- Define las acciones disponibles en el flujo de trabajo de gestion documental de calidad.
-- Catalogo de acciones que pueden ejecutarse sobre los documentos de calidad
-- (crear, revisar, aprobar, rechazar, publicar, archivar, etc.).
--------------------------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS tab_accion_workflow
(
    id_accion_worflow    DECIMAL(2,0)               NOT NULL CHECK (id_accion_worflow BETWEEN 1 AND 99),                                                               -- Identificador unico (1 a 99). Clave primaria.
    nom_accion           VARCHAR(15)                NOT NULL CHECK (nom_accion IN ('CREAR','REVISAR','APROBAR','RECHAZAR','PUBLICAR','ARCHIVAR','OBSOLETAR')), 
    ind_borrado          BOOLEAN                    NOT NULL DEFAULT FALSE,                                                                                    -- Borrado logico.
    PRIMARY KEY (id_accion_worflow)
);
--------------------------------------------------------------------------------------------------------
...
(
    ...
    id_estado_origen     DECIMAL(2,0)               NOT NULL CHECK (id_estado_origen BETWEEN 1 AND 20),            -- Estado antes de la accion.
    id_estado_destino    DECIMAL(2,0)               NOT NULL CHECK (id_estado_destino BETWEEN 1 AND 20),           -- Estado despues de la accion.
    id_usuario           VARCHAR(60)                NOT NULL CHECK (id_usuario ~ '^[A-Za-z0-9._-]{3,60}$'),        -- FK al usuario.
    ...
    FOREIGN KEY (id_documento)      REFERENCES tab_enc_documento(id_documento),
    FOREIGN KEY (id_version)        REFERENCES tab_det_documento(id_version),
    FOREIGN KEY (id_accion_worflow) REFERENCES tab_accion_workflow(id_accion_worflow),
...
(
    ...
    tipo_inverso          BOOLEAN                       NOT NULL,   --true = Natural / false = Jurdíco                                              --tipo de inversor
...
--------------------------------------------------------------------
-- 10.2 CREACION DE TRIGGERS PARA AUDITORIAS TABLAS COBNTABILIDAD --
--------------------------------------------------------------------
...
CREATE OR REPLACE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_contratista
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();
...
CREATE OR REPLACE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_partici_programa
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE OR REPLACE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_programa_salud
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();
...
CREATE OR REPLACE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_categoria_epp 
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();
...
CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_pmtros_marcom
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_etapas_funnel
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_canales
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_motivos_perdida
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_kpis_marcom
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_tipo_campana
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_tipos_interaccion_marcom
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_criterios_segmentacion
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_valores_segmentacion
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_tablas_marcom
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_atributos_marcom
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_campos_segmentacion_marcom
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_reglas_segmentacion
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_condiciones_regla
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_campanas
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_leads
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_evento
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_plantillas_correo
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_campana_canal
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_lead_camp
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_prod_camp
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_even_lead
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_segmentacion_tercero
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_contactos_adicionales
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_presupuesto_campana
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_presupuesto_evento
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_interacciones_lead
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_proximas_acciones
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_medicion_kpi
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_envios
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();
...
CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE OR DELETE ON tab_tip_notas
FOR EACH ROW EXECUTE FUNCTION public.fun_audit_trail();
...
CREATE OR REPLACE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_caja_menor
FOR EACH ROW EXECUTE FUNCTION public.fun_audit_trail();
...
CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_tipo_documento
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_tabla_retencion
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_estado_correspondencia
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_accion_workflow_doc
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_correspondencia
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_indexacion
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_expediente
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_doc_expediente
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_permiso_acceso
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_workflow_expediente
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_tipo_pqrs
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_param_venc_pqrs
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_estado_pqrs
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_motivo_pqrs
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_enc_pqrs
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_det_pqrs
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_seg_pqrs
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_respuesta_pqrs
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_anexo_pqrs
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_normas
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_certificacion
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_estado_documento
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_accion_workflow
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_enc_documento
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_det_documento
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_doc_norma
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_doc_archivo
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_procesos
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_auditorias
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_hallazgo
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_doc_anexo
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_accion_correctiva
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();

CREATE TRIGGER tri_audit_trail AFTER INSERT OR UPDATE ON tab_workflow
FOR EACH ROW EXECUTE FUNCTION fun_audit_trail();
