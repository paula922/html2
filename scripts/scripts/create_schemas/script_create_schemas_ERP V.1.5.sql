...
(
    ...
    FOREIGN KEY(id_periodo) REFERENCES CONPRE.tab_period_presupuesto(id_periodo),       -- FK a período
    FOREIGN KEY(id_tipo_presupuesto) REFERENCES CONPRE.tab_tipo_presupuesto(id_tipo_presupuesto), -- FK a tipo
...
(
    ...
    FOREIGN KEY(id_plan) REFERENCES CONPRE.tab_planeacion_presupuesto(id_plan),          -- FK a plan
...
(
    ...
    FOREIGN KEY(id_presupuesto) REFERENCES CONPRE.tab_presupuesto(id_presupuesto),     -- FK a presupuesto
    FOREIGN KEY(id_nivel, id_cod_puc) REFERENCES CONPRE.tab_puc(id_nivel, id_cod_puc)  -- FK a PUC
);
...
(
    ...
    FOREIGN KEY(id_detalle_presupuesto) REFERENCES CONPRE.tab_det_presupuestos(id_detalle_presupuesto) -- FK a detalle
);
...
CREATE TABLE IF NOT EXISTS CONPRE.tab_puc
(
...
CREATE INDEX IF NOT EXISTS idx_puc_cod_superior ON CONPRE.tab_puc(cod_superior);          -- Para búsquedas jerárquicas
CREATE INDEX IF NOT EXISTS idx_puc_nivel_estado ON CONPRE.tab_puc(id_nivel, ind_estado);  -- Para filtros por nivel y estado

-- TABLA 2: Categorías de Activos Fijos (clasificación para depreciación)
CREATE TABLE IF NOT EXISTS CONPRE.tab_cat_activos
(
...
CREATE INDEX IF NOT EXISTS idx_cat_activos_estado ON CONPRE.tab_cat_activos(ind_estado);  -- Filtro por estado

-- TABLA 4: Períodos de Presupuesto (año + mes)
CREATE TABLE IF NOT EXISTS CONPRE.tab_period_presupuesto
(
    id_periodo          DECIMAL(10,0)   NOT NULL CHECK(id_periodo > 0),                    -- ID del período
    anio_periodo        DECIMAL(4,0)    NOT NULL CHECK(anio_periodo BETWEEN 2000 AND 2100), -- Año (2000-2100)
    mes_periodo         DECIMAL(2,0)    NOT NULL CHECK(mes_periodo BETWEEN 1 AND 12),      -- Mes (1-12)
...
CREATE INDEX IF NOT EXISTS idx_periodo_anio_mes ON CONPRE.tab_period_presupuesto(anio_periodo, mes_periodo); -- Búsqueda por año/mes

-- TABLA 5: Tipos de Presupuesto (Operativo, Inversión, Caja...)
CREATE TABLE IF NOT EXISTS CONPRE.tab_tipo_presupuesto
(
    id_tipo_presupuesto DECIMAL(10,0)   NOT NULL CHECK(id_tipo_presupuesto > 0),           -- ID del tipo
    nom_tipo            VARCHAR(30)     NOT NULL CHECK(LENGTH(nom_tipo) >= 3),             -- Nombre del tipo
    descrip_tipo        VARCHAR(200)    DEFAULT '',                                        -- Descripción
...
CREATE INDEX IF NOT EXISTS idx_tipo_presupuesto_estado ON CONPRE.tab_tipo_presupuesto(ind_estado); -- Filtro por estado


-- TABLA 7: Tipos de Comprobantes (Diario, Ingreso, Egreso)
CREATE TABLE IF NOT EXISTS CONPRE.tab_tip_comprobantes
(
    id_tipcomprobante       SMALLINT        NOT NULL CHECK(id_tipcomprobante > 0),           -- ID del tipo
    nom_tipcomprobante      VARCHAR(30)     NOT NULL CHECK(LENGTH(nom_tipcomprobante) >= 3), -- Nombre
    num_inicial             
...
CREATE INDEX IF NOT EXISTS idx_tip_comprobante_codigo ON CONPRE.tab_tip_comprobantes(cod_tipcomprobante); -- Búsqueda por código

-- TABLA 8: Parámetros Contables (solo UN registro por empresa)
CREATE TABLE IF NOT EXISTS CONPRE.tab_parametros_contab
(
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_parametros_empresa ON CONPRE.tab_parametros_contab(id_empresa); -- Solo UN registro por empresa

-- TABLA 10: Activos Fijos (bienes de la empresa)
CREATE TABLE IF NOT EXISTS CONPRE.tab_act_fijos
(
    ...
    FOREIGN KEY(id_categoria_act) REFERENCES CONPRE.tab_cat_activos(id_categoria_act),  -- FK a categoría
    FOREIGN KEY(id_empresa) REFERENCES public.tab_pmtros_grales(id_empresa),            -- FK a empresa
...
CREATE INDEX IF NOT EXISTS idx_act_fijos_categoria ON CONPRE.tab_act_fijos(id_categoria_act); -- Índice FK

-- TABLA 11: Depreciación de Activos (registro periódico)
CREATE TABLE IF NOT EXISTS CONPRE.tab_depreciacion
(
    ...
    FOREIGN KEY(id_activo) REFERENCES CONPRE.tab_act_fijos(id_activo),                               -- FK a activo
    FOREIGN KEY(id_empresa) REFERENCES public.tab_pmtros_grales(id_empresa),                         -- FK a empresa
    FOREIGN KEY(id_periodo_contable) REFERENCES CONPRE.tab_periodos_contables(id_periodo_contable),  -- FK a período
...
CREATE INDEX IF NOT EXISTS idx_depreciacion_activo ON CONPRE.tab_depreciacion(id_activo); -- Búsqueda por activo
...
CREATE INDEX IF NOT EXISTS idx_presupuesto_periodo ON CONPRE.tab_presupuesto(id_periodo); -- Búsqueda por período

-- TABLA 14: Planeación de Presupuesto (detalle por cuenta y área)
CREATE TABLE IF NOT EXISTS CONPRE.tab_planeacion_presupuesto
(
    AREA
    MES
    CUENTA PUC
...
CREATE INDEX IF NOT EXISTS idx_planeacion_presupuesto ON CONPRE.tab_planeacion_presupuesto(id_presupuesto); -- Búsqueda por presupuesto
CREATE INDEX IF NOT EXISTS idx_planeacion_area ON CONPRE.tab_planeacion_presupuesto(id_area); -- Búsqueda por área
CREATE INDEX IF NOT EXISTS idx_planeacion_puc ON CONPRE.tab_planeacion_presupuesto(id_nivel, id_cod_puc); -- Búsqueda por PUC

-- TABLA 15: Comprobantes Contables (cabecera)
CREATE TABLE IF NOT EXISTS CONPRE.tab_comprobantes_contab
(
    id_tipcomprobante   DECIMAL(10,0)   NOT NULL,                                       -- Tipo (FK)
    NUMERO DE COMPROBANTE
    MES
    ...
    ind_borrado         BOOLEAN         NOT NULL DEFAULT FALSE,                         -- Borrado lógico
    PRIMARY KEY(id_comprobante),                                                       -- Llave primaria
    FOREIGN KEY(id_tipcomprobante) REFERENCES CONPRE.tab_tip_comprobantes(id_tipcomprobante), -- FK a tipo
    FOREIGN KEY(id_empresa) REFERENCES public.tab_pmtros_grales(id_empresa),           -- FK a empresa
    FOREIGN KEY(id_tercero) REFERENCES public.tab_terceros(id_tercero),                -- FK a tercero
    FOREIGN KEY(id_periodo_contable) REFERENCES CONPRE.tab_periodos_contables(id_periodo_contable), -- FK a período
    UNIQUE(id_tipcomprobante, id_periodo_contable, num_comprobante),                   -- No duplicar tipo+período+número
...
CREATE INDEX IF NOT EXISTS idx_comprobantes_fecha ON CONPRE.tab_comprobantes_contab(fecha_comprobante); -- Búsqueda por fecha
CREATE INDEX IF NOT EXISTS idx_comprobantes_estado ON CONPRE.tab_comprobantes_contab(estado); -- Búsqueda por estado

-- TABLA 16: Detalle de Presupuesto (montos comprometidos y ejecutados)
CREATE TABLE IF NOT EXISTS CONPRE.tab_det_presupuestos
(
     id_plan                 DECIMAL(10,0)   NOT NULL,                                     -- Plan (FK)
    monto_aprobado          DECIMAL(15,0)   NOT NULL CHECK(monto_aprobado >= 0),          -- Monto aprobado
    monto_comprometido      DECIMAL(15,0)   NOT NULL DEFAULT 0,                           -- Monto comprometido (pedidos)
...
CREATE INDEX IF NOT EXISTS idx_det_presupuesto_plan ON CONPRE.tab_det_presupuestos(id_plan); -- Búsqueda por plan

-- TABLA 17: Ejecución de Presupuesto (real vs planeado)
CREATE TABLE IF NOT EXISTS CONPRE.tab_ejecucion_presupuesto
(
    id_ejecucion        DECIMAL(10,0)   NOT NULL,                                         -- ID de ejecución
    id_plan             DECIMAL(10,0)   NOT NULL,                                         -- Plan (FK)
    id_comprobante      DECIMAL(10,0)   NOT NULL,                                         -- Comprobante (FK)
...
CREATE INDEX IF NOT EXISTS idx_ejecucion_fecha ON CONPRE.tab_ejecucion_presupuesto(fecha_registro); -- Búsqueda por fecha

-- TABLA 19: Detalle de Comprobantes (partida doble)
CREATE TABLE IF NOT EXISTS CONPRE.tab_det_comprobantes
(
   id_tipcomprobante   DECIMAL(10,0)   NOT NULL,                                       -- Tipo (FK)
    NUMERO DE COMPROBANTE
    CUENTA PUC
    ...
    ind_borrado         BOOLEAN         NOT NULL DEFAULT FALSE,                         -- Borrado lógico
    PRIMARY KEY(id_det_comp),                                                          -- Llave primaria
    FOREIGN KEY(id_comprobante) REFERENCES CONPRE.tab_comprobantes_contab(id_comprobante) ON DELETE CASCADE, -- FK a comprobante (si se borra el comprobante, se borran sus líneas)
    FOREIGN KEY(id_nivel, id_cod_puc) REFERENCES CONPRE.tab_puc(id_nivel, id_cod_puc), -- FK a PUC
    CONSTRAINT chk_det_comprobante_xor CHECK (                                          -- Una línea NO puede tener débito y crédito a la vez
        (valor_debito > 0 AND valor_credito = 0) OR 
        (valor_debito = 0 AND valor_credito > 0)
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_det_comprobantes_unico ON CONPRE.tab_det_comprobantes(id_comprobante, id_nivel, id_cod_puc, linea); -- Evita líneas duplicadas
CREATE INDEX IF NOT EXISTS idx_det_comprobantes_puc ON CONPRE.tab_det_comprobantes(id_nivel, id_cod_puc); -- Búsqueda por cuenta PUC
...
CREATE TABLE IF NOT EXISTS tescxp.tab_pmtros_tescxp
(
...
CREATE TABLE IF NOT EXISTS tescxp.tab_bancoxprov
(
...
CREATE TABLE IF NOT EXISTS tescxp.tab_facturas
(
...
CREATE TABLE IF NOT EXISTS tescxp.tab_progpagos
(
    ...
    FOREIGN KEY         (id_factura)      REFERENCES tescxp.tab_facturas(id_factura) 
);
...
CREATE TABLE IF NOT EXISTS tescxp.tab_pagos
(
    ...
    PRIMARY KEY         (id_pago),
    FOREIGN KEY         (id_factura)            REFERENCES tescxp.tab_facturas(id_factura),
    FOREIGN KEY         (id_proveedor,id_banco) REFERENCES tescxp.tab_bancoxprov(id_proveedor,id_banco)
);
...
CREATE TABLE IF NOT EXISTS conpre.tab_presupuesto_financi
(
...
--    datos_audit             audit_trail,                                                                                                              --pista de auditoría
    ...
    FOREIGN KEY (id_presupuesto) REFERENCES conpre.tab_presupuesto_financi(id_presupuesto)
);
