...
BEGIN
    ...
    -- usuario e IP. Ambos llegan desde PHP (fijar_usuario_audit(),
    --     en audit.php) mediante GUCs de la conexión actual.
    ...
    IF wid_usuario IS NOT NULL THEN
        IF NOT EXISTS (SELECT 1 FROM tab_usuarios WHERE id_usuario = wid_usuario) THEN
            RAISE EXCEPTION 'El usuario "%" registrado en la sesión no existe en tab_usuarios. Operación cancelada.', wid_usuario;
        END IF;
    END IF;
    -----------------------------------------------------------------
    IF (TG_OP = 'INSERT') THEN --insert
        ...
        VALUES (wid_usuario, wip, 'I', '{}'::jsonb, '{}'::jsonb, TG_TABLE_NAME);
        RETURN NEW;
    END IF;
    -----------------------------------------------------------------
    IF (TG_OP = 'DELETE') THEN 
        INSERT INTO tab_audit_trail (id_usuario, ip_conexion, nom_operacion, dato_viejo, dato_nuevo, nom_tabla)
        VALUES (wid_usuario, wip, 'BF', to_jsonb(OLD), '{}'::jsonb, TG_TABLE_NAME); --BF: borrador fisico
        RETURN OLD;
    END IF;
    -----------------------------------------------------------------
   wind_borrado := (to_jsonb(NEW) ? 'ind_borrado');
    ...
    IF wind_borrado THEN
        ...
        IF (wold_borrado = FALSE AND wnew_borrado = TRUE) THEN
            woperacion := 'BL';--borrado logico
        ELSIF (wold_borrado = TRUE AND wnew_borrado = FALSE) THEN
            woperacion := 'R'; --restauracion
        ELSE
            woperacion := 'U'; --update
        END IF;
    ELSE
